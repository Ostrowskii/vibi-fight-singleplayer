...
.story-inventory{gap:12px;padding:18px;}
.story-equipped-row{--story-slot-size:clamp(66px,6vw,72px);display:grid;grid-template-columns:110px minmax(0,1fr);gap:10px;align-items:center;padding:6px 8px;background:rgba(79,48,23,.92);border:1px solid #7b5a32;}
.story-owned-row{display:grid;grid-template-columns:110px minmax(0,1fr);gap:14px;align-items:start;}
.story-hp-card{height:var(--story-slot-size);min-height:0;padding:8px 10px;display:grid;align-content:center;gap:2px;background:#6f4720;border:1px solid rgba(233,208,163,.28);color:#fff4dd;box-sizing:border-box;}
.story-hp-card__label{font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#e0c28d;}
.story-hp-card__value{font-size:26px;line-height:1;}
.story-slot-row{display:grid;grid-template-columns:repeat(3,var(--story-slot-size));gap:8px;justify-self:start;align-self:start;width:max-content;}
...
@media (max-width:900px){
  ...
  .story-equipped-row,.story-owned-row{grid-template-columns:1fr;}
  .story-equipped-row{--story-slot-size:clamp(60px,19vw,64px);gap:8px;padding:6px;}
  .story-slot-row{gap:6px;}
